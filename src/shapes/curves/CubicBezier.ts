...
export class CubicBezier extends Shape {
  ...
  constructor(p0?: Point, p1?: Point, p2?: Point, p3?: Point, id?: string) {
    super(id);
    
    this.p0 = p0 || { x: -50, y: 0 };
    this.p1 = p1 || { x: -30, y: 50 };
    this.p2 = p2 || { x: 30, y: 50 };
    this.p3 = p3 || { x: 50, y: 0 };
  }

  evalLocal(t: number): Point {
    const t1 = 1 - t;
    const x = t1 * t1 * t1 * this.p0.x + 
              3 * t1 * t1 * t * this.p1.x + 
              3 * t1 * t * t * this.p2.x + 
              t * t * t * this.p3.x;
    const y = t1 * t1 * t1 * this.p0.y + 
              3 * t1 * t1 * t * this.p1.y + 
              3 * t1 * t * t * this.p2.y + 
              t * t * t * this.p3.y;
    return { x, y };
  ...
  getApproximationPoints(segments: number = 40): Point[] {
    const points: Point[] = [];
  ...
  getDeviceApproximationPoints(segments: number = 40): Point[] {
  ...
  getBounds(): Bounds {
    const points = this.getDeviceApproximationPoints(40);
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    for (const p of points) {
      minX = Math.min(minX, p.x);
      minY = Math.min(minY, p.y);
  ...
  drawRaster(r: RasterRenderer): void {
    const points = this.getDeviceApproximationPoints(60);
  ...
  setControlPoint(index: number, devicePoint: Point): void {
    ...
    switch(index) {
      ...
      case 3: this.p3 = local; break;
    }
  }

  clone(): CubicBezier {
    const bezier = new CubicBezier(
      { ...this.p0 }, { ...this.p1 }, { ...this.p2 }, { ...this.p3 }, this.id
    );
